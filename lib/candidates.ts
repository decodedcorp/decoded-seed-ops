...
import type { AlternativeImage, ReviewStatus, SeedLook } from "@/types";

type RawPost = { id: string; account: string | null; ts: string; caption_text?: string | null };
...
type RawSeedLookInsertResult = { id: string; source_with_items_image_id: string | null; image_url: string };
type SeedLookSourceRow = { source_with_items_image_id: string | null };
...
export async function buildDraftCandidates(): Promise<{ created: number; skipped: number }> {
  ...
  const existingRows: SeedLookSourceRow[] = [];
  for (const idChunk of chunkArray(targetImageIds, 200)) {
    ...
    if (existingError) {
      throw new ApiError(500, dbErrorMessage("seed_look existing query failed", existingError));
    }
    existingRows.push(...((existing ?? []) as SeedLookSourceRow[]));
  }
  const existingImageIds = new Set(
    existingRows
      .map((row: SeedLookSourceRow) => row.source_with_items_image_id)
      .filter((value): value is string => Boolean(value)),
  );
  ...
  const { data: insertedLooks, error: insertError } = await db
    .from("seed_look")
    .insert(rowsToInsert)
    .select("id,source_with_items_image_id,image_url");

  if (insertError) throw new ApiError(500, dbErrorMessage("seed_look insert failed", insertError));

  const assetRows = ((insertedLooks ?? []) as RawSeedLookInsertResult[])
    .map((look: RawSeedLookInsertResult) => {
      const image = look.source_with_items_image_id
        ? imageById.get(look.source_with_items_image_id)
        : undefined;
...
export async function setCandidateReviewStatus(
  ...
  const { error } = await db.from("seed_look").update(updatePayload).eq("id", candidateId);
  if (error) throw new ApiError(500, error.message);
