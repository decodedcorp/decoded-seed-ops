...
import { deletePublicPostById, getPostSpotSolutionTree } from "@/lib/post-spots-solutions";
...
export async function DELETE(_request: Request, context: { params: Promise<{ postId: string }> }) {
  try {
    ...
    await deletePublicPostById(postId);
    return success({ deleted: true, postId });
  } catch (error) {
    const apiError = toApiError(error);
    return failure("POST_SPOTS_DELETE_POST_FAILED", apiError.message, apiError.status);
  }
}
